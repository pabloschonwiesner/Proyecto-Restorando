...
Listado.prototype.buscarRestaurante = function(id) {
    var restaurantHallado = this.restaurantes.find(restaurant => restaurant.id == id)
    if(restaurantHallado) {
        return restaurantHallado
    } else {
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    this.restaurantes.map(restaurant => arregloCiudades.push(restaurant.ubicacion))

    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse    
    var ciudadesFiltradas = filtrarElementosArray(arregloCiudades);
...
Listado.prototype.obtenerRubros = function() {
    var arregloRubros = [];
    this.restaurantes.map(restaurant => arregloRubros.push(restaurant.rubro))

    var rubrosFiltrados = filtrarElementosArray(arregloRubros)
...
function filtrarElementosArray (vector) {
    var vectorFiltrado = vector.filter((elem, index, self) => {
